...
Surgical Firefox patches applied by Bento's import pipeline for changes that
cannot be expressed via prefs, branding, or bundled extensions.
...
the Bento patch-stack helper consumes that compatibility surface directly with
`git apply`.
...
the helper's deterministic `tiny-glob('**/*.patch', { cwd: 'patches' })` order;
the helper checks this instead of relying on a documented folder order.

Patch format: mail-style `git format-patch` output containing `diff --git`
sections. The helper validates interoperability by replaying the full manifest
series sequentially on a clean Firefox base with
...
manifest base version does not match `bento.json`, `pnpm run import` fails
