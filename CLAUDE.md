...
- **Perf budgets** (CI fails on regression via [scripts/check-size-budgets.mjs](scripts/check-size-budgets.mjs) and direct file limits in [.size-limit.json](.size-limit.json)): shell cold-start JS < 196 KB gz; settings cold-start JS < 190 KB gz; palette/address-bar/confirm/edit-workspace/welcome/menu/workspace-palette/merge-palette cold-start JS < 185–186 KB gz; shell CSS < 35 KB gz; bento-tools background < 40 KB gz; cold-start < 80 ms, tab-switch < 16 ms, sustained 60 fps on panel drag. Tab list virtualized from M1, not M3.
